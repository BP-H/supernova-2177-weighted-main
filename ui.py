...
def main() -> None:
    ...
    # CSS: Pink/black theme, aligned buttons with columns/gaps, sticky sidebar
    st.markdown("""
        <style>
            ...
            .stSidebar button { background-color: rgba(255,255,255,0.05); color: white; border-radius: 20px; padding: 8px 16px; margin: 4px 0; width: 100%; cursor: pointer; border: none; font-size: 14px; transition: 0.2s; }
            .stSidebar button:hover { background-color: rgba(255,20,147,0.2); box-shadow: 0 0 5px #ff1493; }
            .stSidebar .row-widget.stButton { display: flex; justify-content: space-between; gap: 8px; } /* Align buttons in columns */
            ...
            @media (max-width: 768px) { .stSidebar button { padding: 6px 12px; font-size: 12px; } .bottom-nav { padding: 2px 0; } .bottom-nav .stButton > button { font-size: 10px; padding: 1px 2px; } .content-card { grid-template-columns: 1fr; gap: 8px; } }
        </style>
    """, unsafe_allow_html=True)

    # Sidebar: Supernova block at top, profile pic, details, perfectly aligned buttons in columns with gaps
    with st.sidebar:
        st.markdown("""
        ...
        manage_buttons = [
            ("🔬 test_tech", "test_tech"),
            ("🌌 supernNova_2177", "supernova_2177"),
            ("✈️ GLOBALRUNWAY", "globalrunway"),
            ("📂 Show all >", "showall")
        ]
        for i, (label, page) in enumerate(manage_buttons):
            col = manage_cols[i % 2]
            if col.button(label, key=f"manage_{page}", use_container_width=True):
                if page != "showall":
