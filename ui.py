...
# Loader with fallback for "no main/render"
def load_page(page_name: str):
    ...
    if not module_path:
        ...
        # Fallback content
        st.write(f"Placeholder for {page_name.capitalize()} page. Add main() or render() to {page_name}.py.")
        return
    try:
...
# Main (sidebar restored with square style via CSS)
def main() -> None:
    st.set_page_config(
        page_title="superNova_2177",
        layout="wide",
        initial_sidebar_state="expanded"  # Restores visibility
    )
    st.session_state.setdefault("theme", "light")
    initialize_theme(st.session_state["theme"])

    # CSS for square sidebar
    st.markdown("""
        <style>
    ...
    with st.sidebar:
        st.title("🌌 superNova")  # Your square sidebar title
        PAGES = {
            "Feed": "feed",
            "Chat": "chat",
            "Messages": "messages",
