...
(() => {{
  ...
  function animate() {{
    ...
    for (let i = collectibles.length - 1; i >= 0; i--) {{
      ...
      c.position.z += travelSpeed * 0.9; // fly toward camera as we move forward
      if (c.position.z > 50) {{
        // recycle far behind
        c.position.z -= 2200;
  ...
  // after init, set base speed again in case of re-render
  // note: streamlit reruns will recreate iframe, so this is best-effort

  // start
  // init() will be called after gate closes
